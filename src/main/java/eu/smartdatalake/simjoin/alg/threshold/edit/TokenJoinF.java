...
//			hybridLimit = 1 - (1 - threshold) / 2;
		...
		for (int R = 0; R < collection.sets.length; R++) {
			...
			while (it.hasNext()) {
				...
				double localUtilGathered = utilGathered;
				double localSumStopped = querySet.sumStopped;
				long localJointTime = System.nanoTime();
				UB = postJoint(R, S, querySet, idx, persThreshold, localUtilGathered, localSumStopped, posTok);
				localJointTime = System.nanoTime() - localJointTime;

				localUtilGathered = utilGathered;
				localSumStopped = querySet.sumStopped;
				long localJointPosTime = System.nanoTime();
				UB = postPositional(R, S, querySet, idx, persThreshold, localUtilGathered, localSumStopped, posTok);
				localJointPosTime = System.nanoTime() - localJointPosTime;
